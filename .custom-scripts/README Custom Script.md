...
5. Create a file in that custom-script location that has an executable extension (`.cmd` if on windows)
6. Ensure that the custom-scripts directory is on your PATH
7. Add the following instructions to that custom script:

```bash
@echo off
REM add a helpful comment or REMark about the script if you want
bash "<absolutePathToScript>" %*
```
